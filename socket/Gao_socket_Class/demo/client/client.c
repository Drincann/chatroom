...
void data_coming_callback(SOCKET client_sock, char* data) {  //数据到达
  ...
  scanf("%s", buf);  //输入数据
  printf("\nsend:%d",
         (*client.method_Send_msg)(client, buf, strlen(buf)));  //发送
}
...
void main() {
  ...
  {  //置对象成员属性
    ...
    printf("Ip：");
    scanf("%s", client.member_ip);  //输入 server ip地址
    printf("Port：");
    scanf("%hu", &client.member_port);  //输入 server 监听端口
  }

  if (Startup_sock_api() == -1)
    return;  // startup_sock_api() 必须首先调用(同一进程调用一次即可)
