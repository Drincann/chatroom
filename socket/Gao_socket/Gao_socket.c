...
int create_serversock(
  ...
  {  //设置 server_addr
    ...
    server.socket_addr.sin_port =
        port == 0 ? htons(INADDR_ANY)
                  : htons(port);  // htons()（htonl（）长型）将 int
                                  // 转换为主机字节顺序，逆转函数为
                                  // ntohs()（ntohl（）长型），
  }
  ...
  while (1) {
    select(0, &temp_REDfd, NULL, NULL,
           &timv);  // windows 下第一个单数仅用于保持兼容性，可以忽略
    if (close_ser) {
      close_ser = 0;  //重置
    ...
    if (FD_ISSET(server.socket, &temp_REDfd)) {  // client 请求连接
      sock_temp.socket =
...
int send_msg(SOCKET client_sock,
             char* msg,
             int lenth) {  //保证缓冲区有结尾 \0 ,或提供精准的lenth
